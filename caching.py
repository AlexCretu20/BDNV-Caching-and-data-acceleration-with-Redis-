...
class FavoritesWriteThrough:
    ...
    #Sterge favorite (stergere + invalidare)
    def delete_favorite(user_id, place_name: str):
        cache_key = f"favorites:{user_id}"

        # sterge din DB
        favorites_collection.update_one(
            {"user_id": user_id},
            {"$pull": {"places": {"name": place_name}}}
        )

        # invalidare: stergem tot cache ul, urmatorul GET repopuleaza din DB
        redis_favorites.delete(cache_key)

        print(f"Deleted favorite '{place_name}' for user {user_id} -> cache invalidated")
        return True